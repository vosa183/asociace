...
// Zde ideálně použij 'service_role' klíč z nastavení Supabase, aby mohl skript přepisovat data na pozadí
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (!settings || !settings.fio_token) throw new Error('Chybí Fio token v databázi.');
        ...
        for (const t of trans) {
            ...
            if (castka > 0 && vs && zpravaOriginal !== "") {
                ...
                if (regs && regs.length > 0) {
                    for (const reg of regs) {
                        ...
                        if (turnajNazevOrezany !== "" && zpravaOrezana.includes(turnajNazevOrezany)) {
                            ...
                            if (!updateError) {
                                matchedCount++;
                                log.push(`[ÚSPĚCH] VS: ${vs} | Zpráva: "${zpravaOriginal}" | Turnaj: ${turnajNazev}`);
                            } else {
                                log.push(`[CHYBA DB] VS: ${vs} | Nelze zapsat: ${updateError.message}`);
                            }
                ...
                log.push(`[IGNOROVÁNO] Částka: ${castka} | VS: ${vs || 'CHYBÍ'} | Zpráva: "${zpravaOriginal || 'CHYBÍ'}"`);
            }
        }

        return res.status(200).json({ success: true, count: matchedCount, log: log });
